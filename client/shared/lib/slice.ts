...
import { RejectValue, ResetErrorActionPayload, ResetStatusActionPayload, SliceStateStatusName } from "./slice-types";
...
export const pendingHandler = <SliceState>(
    thunkName: SliceStateStatusName<SliceState>
): CaseReducer<SliceState> =>
    (state) => ({ ...state, [`${thunkName}Status`]: 'pending' });

export const rejectedHandler = <SliceState, RV = unknown>(
    thunkName: SliceStateStatusName<SliceState>
...
export const resetSliceStatus = <T>(
    state: T,
    { payload }: ResetStatusActionPayload<T>
) =>
    payload
        .reduce((prev, curr) => ({ ...prev, [curr]: 'idle' as APIStatus }), { ...state });

export const resetSliceError = <T>(
    state: T,
    { payload }: ResetErrorActionPayload<T>
) =>
    payload
