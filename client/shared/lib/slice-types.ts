...
type StripSuffix<S extends string, Suffix extends string> = S extends `${infer Prefix}${Suffix}` ? Prefix : never;

export type SliceStateStatusName<SliceState> = StripSuffix<SliceStateStatus<SliceState> | SliceStateError<SliceState>, 'Status' | 'Error'>;
